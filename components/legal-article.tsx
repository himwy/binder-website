...
export function LegalArticle({ doc }: { doc: LegalDoc }) {
  return (
    <main className="max-w-[720px] mx-auto px-6 md:px-12 py-24">
      <p className="text-xs uppercase tracking-[0.16em] text-muted mb-4">Bindy · Legal</p>
      ...
      <div className="flex flex-col gap-8">
        {doc.sections.map((s) => (
          <section key={s.heading ?? s.text.slice(0, 40)}>
            {s.heading ? (
              <h2 className="text-lg font-bold tracking-[-0.01em] mb-2">{s.heading}</h2>
            ) : null}
      ...
      <p className="text-sm text-muted mt-14 pt-8 border-t border-black/10">
        Questions? Email{" "}
        <a href="mailto:contact@nearmint.tech" className="underline underline-offset-4">
